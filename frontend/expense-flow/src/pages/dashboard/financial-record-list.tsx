...
import { FinancialRecord, useFinancialRecoreds } from "../../context/financial-record-context"
...
const EditableCell: React.FC<EditableCellProps> = ({value: initialValue, row,column,updateRecord, editable}) => {
    ...
    >
        {isEditing ? 
            <input value={value} onBlur= {onBlur} onChange={(e) => setValue(e.target.value)} autoFocus style={{width: "100%"}}/> 
            : (typeof value === "string" ? (value) : (value.toString()))}
    </div>)
}

export const FinancialRecordList = () => {
    const { records,updateRecord,deleteRecord } = useFinancialRecoreds();
